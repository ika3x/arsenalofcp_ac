...
class SortedList implements Iterable<Long> {
    ...
    public PrimitiveIterator.OfLong rangedIterator(long start, long end) {
        return new RangeItr(start, end, false);
    }

    public PrimitiveIterator.OfLong rangeClosedIterator(long start, long end) {
        return new RangeItr(start, end, true);
    }

    private final class RangeItr implements PrimitiveIterator.OfLong {
        private int bi;
        private int pos;
        ...
        RangeItr(long start, long end, boolean inclusive) {
            this.end = end;
            this.inclusive = inclusive;

            if (blocks.isEmpty()) { finished = true; return; }

            int b = findFirstBlockWithLastGe(start); // O(log m)
            if (b < 0) { finished = true; return; }

            bi  = b;
            pos = blocks.get(bi).lowerBound(start);   // O(log blockSize)
            skipEmptyBlocks();
        }

        private void skipEmptyBlocks() {
            while (!finished && pos >= blocks.get(bi).size()) {
                if (bi + 1 < blocks.size()) { bi++; pos = 0; }
                else finished = true;
            }
        }
        ...
        public boolean hasNext() {
            ...
            long v = blocks.get(bi).get(pos);
            if (inclusive ? v > end : v >= end) {
                finished = true;
                return false;
            }
            return true;
