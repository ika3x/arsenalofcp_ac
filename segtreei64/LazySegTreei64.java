...
class LazySegTreei64 {
    @FunctionalInterface
    public interface BiOperatori64 {
        long apply(long val1, long val2);
    }
    
    @FunctionalInterface
    public interface FSMapper {
        long apply(long fVal, long sVal, long sSize);
    }

    @FunctionalInterface
    public interface BinarySearchBool {
        boolean test(long v);
    }
    ...
    final int N;
    final int Log;
    final BiOperatori64 Op;
    final long E;
    final FSMapper Mapping;
    final BiOperatori64 Composition;
    final long Id;
    ...
    public LazySegTreei64(int n, BiOperatori64 op, long e, FSMapper mapping, BiOperatori64 composition, long id) {
        this.MAX = n;
        int k = 1;
        while (k < n) k <<= 1;
        this.N = k;
    ...
    public LazySegTreei64(long[] dat, BiOperatori64 op, long e, FSMapper mapping, BiOperatori64 composition, long id) {
    ...
    private void push(int k) {
        ...
        Dat[lk] = Mapping.apply(Laz[k], Dat[lk], Siz[lk]);
        Dat[rk] = Mapping.apply(Laz[k], Dat[rk], Siz[lk]);
    ...
    public void apply(int p, long f) {
        ...
        p += N;
        pushTo(p);
        Dat[p] = Mapping.apply(f, Dat[p], Siz[p]);
        updateFrom(p);
    }
    ...
    public void apply(int l, int r, long f) {
        ...
        for (int l2 = l, r2 = r; l2 < r2;) {
            if ((l2 & 1) == 1) {
                Dat[l2] = Mapping.apply(f, Dat[l2], Siz[l2]);
                if (l2 < N) Laz[l2] = Composition.apply(f, Laz[l2]);
                l2++;
            }
            if ((r2 & 1) == 1) {
                r2--;
                Dat[r2] = Mapping.apply(f, Dat[r2], Siz[r2]);
    ...
    public int maxRight(int l, BinarySearchBool g) {
        inclusiveRangeCheck(l);
        if (!g.test(E)) {
    ...
    public int minLeft(int r, BinarySearchBool g) {
        inclusiveRangeCheck(r);
        if (!g.test(E)) {
            throw new IllegalArgumentException("Identity element must satisfy the condition.");
        }
    ...
    private long[] simulatePushAll() {
        ...
        long[] simSiz = java.util.Arrays.copyOf(Siz, 2 * N);
        for (int k = 1; k < N; k++) {
            if (simLaz[k] == Id) continue;
            int lk = k << 1 | 0, rk = k << 1 | 1;
            simDat[lk] = Mapping.apply(simLaz[k], simDat[lk], simSiz[lk]);
            simDat[rk] = Mapping.apply(simLaz[k], simDat[rk], simSiz[rk]);
